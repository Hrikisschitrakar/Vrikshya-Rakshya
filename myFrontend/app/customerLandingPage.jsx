...
const CustomerLandingPage = () => {
  ...
  // Updated analyzePlantLeaf with plant name check
  const analyzePlantLeaf = async (uri) => {
    setAnalyzing(true);
    setResult(null);
    ...
    try {
      ...
      // Check if plantName is in allowed list
      const validPlants = ['Apple', 'Corn_(maize)', 'Tomato', 'Potato'];
      if (!validPlants.includes(plantName)) {
        setResult({ status: 'error', message: 'Sorry, cannot predict this plant image. Please try another image.' });
        setAnalyzing(false);
        return;
      }
