...
const VendorProfileScreen = () => {
  ...
  const fetchProfileImage = async () => {
    try {
      ...
      setVendor((prevVendor) => ({
        ...prevVendor,
        profileImage: `http://127.0.0.1:8000${image_url}`, // Update profile image
        coverImage: `http://127.0.0.1:8000${image_url}`,
      }));
  ...
  const handleImageUpload = async () => {
    try {
      await axios.post('http://127.0.0.1:8000/profile', imageForm);
      setVendor((prevVendor) => ({
        ...prevVendor,
  ...
  useEffect(() => {
    const fetchVendorData = async () => {
      try {
        const vendorResponse = await axios.get(`http://127.0.0.1:8000/vendor/profile/${username}`);
    ...
    const fetchStatistics = async () => {
      try {
        const statsResponse = await axios.get(`http://127.0.0.1:8000/vendor/dashboard/${username}`);
        setStatistics({
  ...
  const handleEditSubmit = async () => {
    try {
      await axios.put(`http://127.0.0.1:8000/vendor/profile/${username}`, editForm);
              ...
              <ScrollView>
                {products.map((product, index) => (
                  <View key={index} style={styles.productItem}>
                    <Image
                      source={{ uri: `http://127.0.0.1:8000${product.image_url}` }}
