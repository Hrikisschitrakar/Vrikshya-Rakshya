...
const VendorHomePage = () => {
  ...
  const fetchVendorProducts = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/products/${username}`);
  ...
  const fetchVendorStats = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/vendor/dashboard/${username}`);
  ...
  const fetchRecentOrders = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/orders/vendor-history/${username}`);
  ...
  const handleAddProduct = async () => {
    ...
    try {
      await axios.post(`http://127.0.0.1:8000/products/${username}`, formData, {
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:8000/products/${username}/${productIdToDelete}`);
  ...
  const handleRestock = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://127.0.0.1:8000/restock?product_id=${payload.product_id}&quantity=${payload.quantity}`
      );
  ...
  const handleUpdatePrice = async () => {
    ...
    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/price?product_id=${payload.product_id}&price=${payload.price}`
      );
        ...
        {loading ? (
          ...
          products.map((product, index) => (
            <View key={index} style={[styles.productCard, index === products.length - 1 ? null : styles.productCardBorder]}>
              <Image 
                source={{ uri: `http://127.0.0.1:8000${product.image_url}` || 'https://via.placeholder.com/50' }}
