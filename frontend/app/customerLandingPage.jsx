      ...
      <View style={styles.productInfo}>
        ...
        <View style={styles.productPriceRow}>
          <TouchableOpacity 
            ...
            onPress={async () => {
              try {
                const response = await fetch(`http://127.0.0.1:8000/wishlist/${username}/${item.id}`, {
                  method: 'POST',
                });
                if (response.ok) {
  ...
  const saveRemedy = async (username, diseaseName) => {
    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/save-remedy?username=${encodeURIComponent(username)}&disease_name=${encodeURIComponent(diseaseName)}`
      );
